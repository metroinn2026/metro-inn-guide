const IMG = {
  ...
  valley:
    "https://commons.wikimedia.org/wiki/Special:FilePath/Thermal%20Valley%20in%20Beitou.jpg",
  museum:
    "https://commons.wikimedia.org/wiki/Special:FilePath/Beitou%20Hot%20Spring%20Museum.jpg",
  library:
    "https://commons.wikimedia.org/wiki/Special:FilePath/Taipei%20Public%20Library%20Beitou%20Branch.jpg",
  yang: "https://commons.wikimedia.org/wiki/Special:FilePath/Yangmingshan%20National%20Park.jpg",
  shilin:
    "https://commons.wikimedia.org/wiki/Special:FilePath/Taiwan%20Shilin%20Night%20Market.jpg",
  tamsui:
    "https://commons.wikimedia.org/wiki/Special:FilePath/2020%20Sunset%20of%20Tamsui%20Fisherman%27s%20Wharf.jpg",
  onsen:
    "https://commons.wikimedia.org/wiki/Special:FilePath/Beitou%20Thermal%20Valley.jpg",
  dining:
  ...
  grill:
    "https://commons.wikimedia.org/wiki/Special:FilePath/Taiwan%20Night%20Market.JPG",
  gift: "https://commons.wikimedia.org/wiki/Special:FilePath/Raohe%20Street%20Night%20Market%2C%20Taipei%2020170923.jpg",
  hotel:
    "https://commons.wikimedia.org/wiki/Special:FilePath/2007-10-23%20Room%20%40%20Far%20East%20Plaza%20Hotel%2C%20Taipei%2001.jpg",
  family:
...
document.addEventListener("click", (event) => {
  ...
  if (toggle) {
    ...
    !event.target.closest("#mobile-site-menu") ||
    event.target.closest("#mobile-site-menu a")
  ) {
...
function renderBreadcrumb(category, categoryUrl, title) {
  ...
  section.href = categoryUrl;
  section.textContent = category;
  const sep = () => {
    const x = document.createElement("span");
    x.className = "breadcrumb-separator";
    x.textContent = "〉";
  ...
  if (title) {
    ...
    node.append(sep(), current);
  }
  node.setAttribute("aria-label", "麵包屑導覽");
}
function footer() {
  return `<footer class="footer"><div class="container"><div class="footer-compact"><div class="footer-about"><h2 id="footerTitle">北投旅圖</h2><p id="footerDescription">從一間舒服的旅店開始，慢慢走向你喜歡的城市風景。</p><address class="footer-contact">北捷行旅（捷運北投會館）<br>地址：<a href="https://www.google.com/maps/search/?api=1&amp;query=%E5%8F%B0%E5%8C%97%E5%B8%82%E5%8C%97%E6%8A%95%E5%8D%80%E5%A4%A7%E6%A5%AD%E8%B7%AF527%E5%B7%B788%E8%99%9F" target="_blank" rel="noopener noreferrer">台北市北投區大業路527巷88號</a>（捷運北投園區內）<br>訂房專線：<a href="tel:+886228921485">02-2892-1485</a>（24小時）</address><div id="footerSocial" class="footer-social"></div></div><div class="footer-newsletter"><button class="footer-newsletter-toggle" type="button" aria-expanded="false" aria-controls="footerNewsletterPanel"><span>✉　訂閱電子報</span><span class="footer-newsletter-chevron" aria-hidden="true">＋</span></button><div class="footer-newsletter-panel" id="footerNewsletterPanel" hidden><p>掌握住宿優惠與旅遊活動資訊。</p><form class="footer-newsletter-form" id="footerNewsletterForm"><label class="sr-only" for="footerNewsletterEmail">Email</label><div class="footer-newsletter-row"><input id="footerNewsletterEmail" type="email" autocomplete="email" maxlength="254" placeholder="請輸入 Email" required><button type="submit">訂閱</button></div><label class="footer-newsletter-consent"><input id="footerNewsletterConsent" type="checkbox" required><span>我同意接收北投旅圖的活動、住宿優惠及旅遊資訊，並可隨時取消訂閱。</span></label><p class="footer-newsletter-msg" id="footerNewsletterMsg" role="status" aria-live="polite"></p></form></div></div></div><p id="footerCopy" class="copy">© 2026 臺北大眾捷運股份有限公司。版權所有。</p></div></footer>`;
}
// 所有頁面共用的電子報展開與訂閱行為。
document.addEventListener("click", (event) => {
  const toggle = event.target.closest(".footer-newsletter-toggle");
  if (!toggle) return;
  const panel = document.getElementById("footerNewsletterPanel");
  if (!panel) return;
  const open = toggle.getAttribute("aria-expanded") !== "true";
  toggle.setAttribute("aria-expanded", String(open));
  panel.hidden = !open;
  toggle.querySelector(".footer-newsletter-chevron").textContent = open
    ? "－"
    : "＋";
});

document.addEventListener("submit", async (event) => {
  if (event.target.id !== "footerNewsletterForm") return;
  event.preventDefault();
  const form = event.target;
  const email = form.querySelector("#footerNewsletterEmail").value.trim();
  const consent = form.querySelector("#footerNewsletterConsent").checked;
  const message = form.querySelector("#footerNewsletterMsg");
  const button = form.querySelector('button[type="submit"]');
  if (!consent) {
    message.textContent = "請先勾選同意接收電子報。";
    return;
  }
  button.disabled = true;
  message.textContent = "訂閱處理中…";
  try {
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/rpc/newsletter_subscribe`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) throw Error(result?.message || "訂閱失敗");
    form.reset();
    message.textContent = "訂閱成功！之後可透過每封電子報下方的連結取消訂閱。";
  } catch (error) {
    message.textContent = "目前無法完成訂閱，請稍後再試。";
    console.error(error);
  } finally {
...
async function loadSiteSettings() {
  ...
  try {
    ...
    if (
      logo &&
      typeof s.logo_url === "string" &&
      /^https:\/\//i.test(s.logo_url.trim())
    )
      logo.src = s.logo_url.trim();
    ...
    setText(
      "footerBooking",
      s.booking_text ? s.booking_text.replace("↗", "").trim() : "",
    );
    ...
    if (social) {
      social.innerHTML = [
        s.facebook_url
          ? `<a href="${s.facebook_url}" target="_blank">Facebook</a>`
          : "",
        s.instagram_url
          ? `<a href="${s.instagram_url}" target="_blank">Instagram</a>`
          : "",
        s.line_url ? `<a href="${s.line_url}" target="_blank">LINE</a>` : "",
        s.google_map_url
          ? `<a href="${s.google_map_url}" target="_blank">Google Map</a>`
          : "",
      ]
        .filter(Boolean)
        .join("");
    ...
    console.warn("site settings not loaded", e);
  }
}
document.addEventListener("DOMContentLoaded", () =>
  setTimeout(loadSiteSettings, 0),
);
...
(function () {
  if (
    ...
    !document.querySelector("script[data-reading-controls]")
  ) {
    var s = document.createElement("script");
    s.src = "font-size-controls.js";
    s.defer = true;
    s.dataset.readingControls = "";
